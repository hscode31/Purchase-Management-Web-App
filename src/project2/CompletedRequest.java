...
public class CompletedRequest extends HttpServlet 
{
            ...
            */
			...
			if(((String)session.getAttribute("post")).equals("hod"))
			{
				...
				while(rs.next())
				{
					...
					list[count][1] = rs.getString("reqid");
					count++;
				}
			...
			{
				String s=(String)session.getAttribute("appid");
				ps = con.prepareStatement("select reqid,user_name from sample.req_slip where self_appr=1 and user_id like"+" "+s);
			...
			if(((String)session.getAttribute("post")).equals("pm"))
			{
				...
				{
					list[count][0] = rs.getString("user_name");
					list[count][1] = rs.getString("req_id");
					System.out.println(list[count]);
					count++;
				}
				session.setAttribute("count",Integer.toString(count));
			...
			{
				ps = con.prepareStatement("select reqid,user_name from sample.req_slip where pm_appr=1");
				rs = ps.executeQuery();
				int count = 0;
				while(rs.next())
				{
